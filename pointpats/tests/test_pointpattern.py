...
class TestPointPattern:
    ...
    def test_point_pattern_lambda_mbb(self):
        np.testing.assert_allclose(self.pp.lambda_mbb, 0.0015710507711240867, RTOL)

    def test_point_pattern_lambda_hull(self):
        np.testing.assert_allclose(self.pp.lambda_hull, 0.0022667153468973137, RTOL)
    ...
    def test_point_pattern_knn(self):
        knn = self.pp.knn(1)
        nn = np.array([[9], [3], [4], [7], [2], [9], [11], [3], [5], [5], [5], [6]])
        nnd = np.array(
            [
                [25.59050019],
                [15.64542745],
    ...
    def test_point_pattern_explode(self):
        ...
        for ppattern in explosion:
            np.testing.assert_array_equal(
                ppattern.df.iloc[0], self.pp.df.loc[ppattern.df.index[0]]
            )

    def test_point_pattern_flip_coordinates(self):
        pp_flip = PointPattern(
            self.points, coord_names=["The x coordinate", "The y coordinate"]
        )
